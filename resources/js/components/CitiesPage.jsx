...
export default function CitiesPage({ token }) {
    ...
    const fetchCities = useCallback(async () => {
        ...
        setError(null);
        try {
            let url = `/api/cities?page=${page}&pageSize=10`;
            if (filterStateId) url += `&stateId=${filterStateId}`;
        ...
        <div>
            <div className="flex items-center justify-between mb-6">
                <div>
                    ...
                    <p className="text-sm text-gray-500 dark:text-gray-400 mt-1">
                        <code className="bg-gray-100 dark:bg-gray-800 px-1.5 py-0.5 rounded text-xs">GET /api/cities</code>
                        {meta && <span className="ml-2">{meta.total} total</span>}
                    </p>
                </div>
                <button
                    onClick={() => { setSelected(null); fetchCities(); }}
            ...
            {!selected && (
                <div className="mb-4 flex items-center gap-2">
                    <label className="text-sm text-gray-500 dark:text-gray-400">Filter by State ID:</label>
                    <input
                        type="text"
                        value={filterStateId}
                        onChange={(e) => { setFilterStateId(e.target.value); setPage(1); }}
                        placeholder="Enter stateId..."
                        className="text-sm bg-white dark:bg-gray-900 border border-gray-200 dark:border-gray-700 rounded px-2.5 py-1.5 w-64 font-mono"
                    />
                    {filterStateId && (
                        <button
                            onClick={() => { setFilterStateId(''); setPage(1); }}
            ...
            {selected ? (
                <div>
                    ...
                    <div className="bg-white dark:bg-gray-900 border border-gray-200 dark:border-gray-800 rounded-lg p-6">
                        <h3 className="text-lg font-semibold mb-1">
                            <code className="text-xs text-gray-500 dark:text-gray-400 font-normal">GET /api/cities/{selected.id}</code>
                        </h3>
                ...
                <div className="text-center py-12 text-gray-400">No cities found. Create some via the API.</div>
            ) : (
                <>
                        ...
                        <table className="w-full text-sm">
                            <thead>
                                <tr className="border-b border-gray-200 dark:border-gray-800 bg-gray-50 dark:bg-gray-950">
                                    ...
                                    <th className="text-left px-4 py-2.5 font-medium text-gray-500 dark:text-gray-400">State ID</th>
                            ...
                            <tbody>
                                {cities.map((c) => (
                                    ...
                                    >
                                        ...
                                        <td className="px-4 py-3 font-mono text-xs text-gray-500">{c.stateId}</td>
